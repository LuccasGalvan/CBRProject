function [retrieved_indexes, similarities, new_case] = retrieve(case_library, new_case, threshold)
    ...
    weighting_factors = [2 5 3 3 2 3 5 2]; %HERE
    ...
    for i=1:size(case_library,1)
        ...
        %multiplicação de vetores com a transposicao = somatorio da
        %multiplicação
        DG = (distances * weighting_factors')/sum(weighting_factors); %HERE 
        final_similarity = 1 - DG; %HERE
...
function [holiday_type_sim] = get_holiday_type_similarities()
    ...
    holiday_type_sim.similarities = [
        % Active Bathing City Education Language Recreation Skiing Wandering
          1.0    0.7     0.7  0.5       0.5      0.6        0.8    0.7       % Active
          0.7    1.0     0.3  0.3       0.2      0.6        0.1    0.4       % Bathing
          0.7    0.3     1.0  0.6       0.5      0.7        0.2    0.8       % City
          0.5    0.3     0.6  1.0       0.8      0.3        0.3    0.5       % Education
          0.5    0.2     0.5  0.8       1.0      0.6        0.2    0.6       % Language
          0.6    0.6     0.7  0.3       0.6      1.0        0.6    0.4       % Recreation
          0.8    0.1     0.2  0.3       0.2      0.6        1.0    0.5       % Skiing
          0.7    0.4     0.8  0.5       0.6      0.4        0.5    1.0       % Wandering
    ];
end
...
function [accommodation_sim] = get_accommodation_similarities()
    ...
    accommodation_sim.similarities = [
        % FiveStars FourStars HolidayFlat OneStar ThreeStars TwoStars 
          1.0       0.8       0.1         0.2     0.6        0.4      % FiveStars
          0.8       1.0       0.2         0.4     0.8        0.6      % FourStars
          0.1       0.2       1.0         0.8     0.6        0.7      % HolidayFlat
          0.2       0.4       0.8         1.0     0.6        0.8      % OneStar
          0.6       0.8       0.6         0.6     1.0        0.8      % ThreeStars
          0.4       0.6       0.7         0.8     0.8        1.0      % TwoStars
    ];
end
...
function [res] = calculate_local_distance(sim, val1, val2)
    ...
    res = 1 - sim.similarities(i1, i2); %HERE
end
